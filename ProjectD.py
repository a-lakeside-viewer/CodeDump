...
- Input line + Send + Ctrl+C shortcut button (sends 0x03) + Ctrl+C keyboard shortcut
...
- Optional "Save as Reference" for a command button's result (baseline saved to text file)
- "Compare Results" side-by-side diff (current vs baseline) with line highlight:
    - Green: inserted (new only)
    - Red: deleted (old only)
    - Yellow: modified (both sides)

Dependencies:
    pip install PyQt5 pyserial

Run:
    python teraterm_like_advanced.py
"""
...
class ProfileEditorDialog(QtWidgets.QDialog):
    ...
    def _save_and_close(self):
        ...
        # commands order for current section
...
class TerminalWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        # context of the last button-initiated command (for reference/compare)
        self._last_btn_profile: Optional[str] = None
        self._last_btn_section: Optional[str] = None
        self._last_btn_label: Optional[str] = None
    ...
    def _build_ui(self):
        ...
        # NEW: reference & compare
        self.save_ref_btn = QtWidgets.QPushButton("Save as Reference")
        self.compare_btn = QtWidgets.QPushButton("Compare Results")
        ...
        action_row.addWidget(self.save_ref_btn)
        ...
        # NEW: Save reference & compare
        self.save_ref_btn.clicked.connect(self._save_command_reference)
    ...
    def _send_input(self):
        ...
        if self.worker and self._connected:
            ...
            # capture for last result (typed command has no label/section context)
            self.last_command = text
            self._result_start_offset = len(self.terminal.toPlainText())
            self._last_btn_profile = None
            self._last_btn_section = None
            self._last_btn_label = None
        else:
    ...
    def _send_ctrl_c(self):
        payload = b"\x03"  # ASCII ETX
        if self.worker and self._connected:
            self.worker.write(payload)
            self.last_command = 'Ctrl+C'
            self._result_start_offset = len(self.terminal.toPlainText())
            # Ctrl+C is not tied to a command button
            self._last_btn_profile = None
            self._last_btn_section = None
            self._last_btn_label = None
        else:
    ...
    def _send_command(self, cmd_text: str, *, btn_label: Optional[str]=None,
                      btn_section: Optional[str]=None, btn_profile: Optional[str]=None):
        if not cmd_text:
            return
        ...
        if self.worker and self._connected:
            ...
            self._result_start_offset = len(self.terminal.toPlainText())
            # record button context for Save Reference / Compare
            self._last_btn_profile = btn_profile
            self._last_btn_section = btn_section
            self._last_btn_label = btn_label
        else:
            QtWidgets.QMessageBox.information(self, 'Not connected', 'Connect to a serial port first.')
    ...
    def _load_profiles(self):
        ...
        if not names:
            ...
            p = Profile(name='Default', sections={
                'Quick': [
                    CommandItem(label='AT', text='AT'),
                    CommandItem(label='Reset', text='reset')
                ]
            })
            self.store.save_profile(p)
            names = [p.name]
    ...
    def _make_button(self, section_name: str, cmd: CommandItem) -> QtWidgets.QPushButton:
        btn = QtWidgets.QPushButton(cmd.label or '(no label)')
        btn.setMinimumHeight(36)
        btn.clicked.connect(
            lambda _, c=cmd, s=section_name, p=self.current_profile.name:
                self._send_command(c.text, btn_label=c.label, btn_section=s, btn_profile=p)
        )
        return btn
    ...
    def _rebuild_buttons(self, commands: List[CommandItem]):
        ...
        if not commands:
            ...
            return
        for cmd in commands:
            layout.addWidget(self._make_button(self.section_combo.currentText(), cmd))
        layout.addStretch(1)
    ...
    def _on_search_changed(self, text: str):
        ...
        for sec_name, cmd in matches:
            ...
            btn.setMinimumHeight(36)
            btn.clicked.connect(
                lambda _, c=cmd, s=sec_name, p=self.current_profile.name:
                    self._send_command(c.text, btn_label=c.label, btn_section=s, btn_profile=p)
            )
    ...
    # ---------------------- Save Reference & Compare ----------------------
    def _current_result_text(self) -> str:
        """Return the current result captured since last send (trimmed)."""
        full = self.terminal.toPlainText()
        if self._result_start_offset <= 0 or self._result_start_offset > len(full):
            return ''
        res = full[self._result_start_offset:]
        return res.rstrip()

    def _reference_file_path(self) -> Optional[Path]:
        """Build path profiles/<profile>/<section>/<label>.txt for last button command."""
        if not (self._last_btn_profile and self._last_btn_section and self._last_btn_label):
            return None
        base = Path(PROFILES_DIR) / self._last_btn_profile / self._last_btn_section
        # sanitize label into filename
        safe_label = "".join(c for c in self._last_btn_label if c not in "\\/:*?\"<>|").strip()
        if not safe_label:
            safe_label = "command"
        return base / f"{safe_label}.txt"

    def _save_command_reference(self):
        if not (self._last_btn_label and self._last_btn_section and self._last_btn_profile):
            QtWidgets.QMessageBox.information(self, "Info", "Run a command button first to establish context.")
            return
        result = self._current_result_text()
        ref_path = self._reference_file_path()
        if ref_path is None:
            QtWidgets.QMessageBox.information(self, "Info", "No valid command context.")
            return
        ref_path.parent.mkdir(parents=True, exist_ok=True)
        try:
            ref_path.write_text(result, encoding="utf-8")
            QtWidgets.QMessageBox.information(self, "Reference Saved",
                                              f"Saved reference for '{self._last_btn_label}' at:\n{ref_path}")
        except Exception as e:
            QtWidgets.QMessageBox.warning(self, "Error", f"Failed to save reference:\n{e}")
    ...
    def _compare_results(self):
        ...
        if ref_path is None or not ref_path.exists():
            QtWidgets.QMessageBox.information(self, "Info", "No reference saved yet for this command.")
            return
        try:
            old_text = ref_path.read_text(encoding="utf-8")
        except Exception as e:
            QtWidgets.QMessageBox.warning(self, "Error", f"Failed to load reference:\n{e}")
            return
        new_text = self._current_result_text()
        self._show_diff_dialog(self._last_btn_label, old_text, new_text)

    def _show_diff_dialog(self, label: str, old_text: str, new_text: str):
        dialog = QtWidgets.QDialog(self)
        dialog.setWindowTitle(f"Compare Results: {label}")
        layout = QtWidgets.QVBoxLayout(dialog)

        splitter = QtWidgets.QSplitter()
        old_edit = QtWidgets.QPlainTextEdit(old_text); old_edit.setReadOnly(True)
        new_edit = QtWidgets.QPlainTextEdit(new_text); new_edit.setReadOnly(True)
        # monospaced for alignment
        mono_css = "font-family: Consolas, 'Courier New', monospace; font-size: 11pt;"
        old_edit.setStyleSheet(mono_css); new_edit.setStyleSheet(mono_css)
        splitter.addWidget(old_edit); splitter.addWidget(new_edit)
        layout.addWidget(splitter)

        # legend
        legend = QtWidgets.QLabel("Legend:  Added (new only)=Green   Deleted (old only)=Red   Modified (both)=Yellow")
        layout.addWidget(legend)

        self._highlight_differences(old_edit, new_edit, old_text, new_text)
        dialog.resize(1000, 620)
        dialog.exec_()

    def _highlight_differences(self, old_edit: QtWidgets.QPlainTextEdit,
                               new_edit: QtWidgets.QPlainTextEdit,
                               old: str, new: str):
        """
        Highlight differences per-line in two editors using difflib.SequenceMatcher opcodes.
        - replace -> yellow on both sides (fmt_mod)
        - delete  -> red on old side only (fmt_del)
        - insert  -> green on new side only (fmt_add)
        """
        matcher = difflib.SequenceMatcher(None, old.splitlines(), new.splitlines())

        fmt_add = QtGui.QTextCharFormat(); fmt_add.setBackground(QtGui.QColor("lightgreen"))
        fmt_del = QtGui.QTextCharFormat(); fmt_del.setBackground(QtGui.QColor("lightcoral"))
        fmt_mod = QtGui.QTextCharFormat(); fmt_mod.setBackground(QtGui.QColor("khaki"))

        opcodes = matcher.get_opcodes()
        self._apply_formats(old_edit, opcodes, side="old", fmt_add=fmt_add, fmt_del=fmt_del, fmt_mod=fmt_mod)
        self._apply_formats(new_edit, opcodes, side="new", fmt_add=fmt_add, fmt_del=fmt_del, fmt_mod=fmt_mod)

        # Optionally, scroll both to top for consistency
        old_edit.moveCursor(QtGui.QTextCursor.Start)
        new_edit.moveCursor(QtGui.QTextCursor.Start)

    def _apply_formats(self, edit: QtWidgets.QPlainTextEdit, opcodes, side: str,
                       fmt_add: QtGui.QTextCharFormat,
                       fmt_del: QtGui.QTextCharFormat,
                       fmt_mod: QtGui.QTextCharFormat):
        doc = edit.document()
